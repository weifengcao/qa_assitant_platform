...
1. `Stats`: answer "status/count/latency/usage..." using read-only tools.
1. `Security`: enforce org/user permissions and reduce sensitive leakage (RBAC + redaction baseline).
...
1. RAG for how-to: hybrid retrieval (`BM25 + embeddings`) with reranking
1. Tool calling for stats: typed tools with JSON Schemas
1. Policy checks before retrieval/tool execution: RBAC + tenant scope
1. Stable response contract: `answer + citations + actions + warnings`
...
1. Run RBAC/policy checks and select best permitted tool by intent/keyword match
1. Execute connector (read-only by default)
1. Post-process (units, rounding, suppression)
1. Compose response
